...
This section is a literature study of the article \cite{LVV09}. The goal of the authors is to show that for practical applications, the solution of the Timoshenko beam model is close to the solution of a two-dimensional beam model. This is referred to as the validity of the Timoshenko beam model in this dissertation.\\

For practical applications, it would be preferred to compare the Timoshenko beam model to a more realistic three-dimensional model. But the authors of \cite{LVV09} explain that this will introduce complexities. \textcolor{red}{What complexities?} To mitigate some of the complexities, the two-dimensional model is used as an intermediate step.\\

Where \cite{LVV09} differs from most other research, is that the boundary conditions are also taken into consideration. The authors of \cite{LVV09} use a cantilever beam configuration for their investigation.\\

To compare the two models, the authors compare the natural frequencies of the models. From modal analysis, it is known that a solution of a second order hyperbolic type problem is a linear combination of the eigenvalues and eigenfunctions of the model. Therefore it is sufficient to compare the eigenvalues and eigenfunctions to determine the validity of the different dimensional models.\\

\subsection{The models}
Consider the cantilever Timoshenko beam model problem Problem T-2 from section \ref{ssec:1D_Model:ModelProblems} and the cantilever two-dimensional beam model problem Problem 2D-1 from section \ref{ssec:2D_Model:ModelProblem}.\\

Figure 6.1 shows the two beams side-by-side for a visual guide.
...
The parameter $h$ is the height of the two-dimensional beam. Recall the dimensionless constant $\displaystyle \alpha = \frac{A \ell^2}{I}$ from section \ref{sssec:1D_Model:DimensionlessForm}. For the Timoshenko beam model with a rectangular cross-section, the area moment of inertial is 
\begin{eqnarray*}
	I = \frac{h^3b}{12}.
\end{eqnarray*}

After substitution, the following relation ship between $\alpha$ and $h$ is found
\begin{eqnarray*}
	h = \sqrt{\frac{12}{\alpha}}.
\end{eqnarray*}
The parameter $\alpha$ can be chosen in such a way that the beams have equal dimensions.


\subsection{Accuracy of the numerical eigenvalues}
For the eigenvalues and eigenfunctions of the Timoshenko beam, the authors of \cite{LVV09} reference an another article \cite{VV06}. In this article \cite{VV06}, a method is described to obtain the eigenvalues and eigenfunctions of a (cantilever) Timoshenko beam. This method also allows for the eigenvalues to be obtained to a desired level of accuracy (using a numerical method such as interval division). In this dissertation, \cite{VV06} is discussed in section \ref{sec:Timo:EigenvalueProblem}.\\

For the two-dimensional beam, the Finite Element Method is used to calculate the eigenvalues and eigenfunctions. In \cite{LVV09} the authors used piecewise bi-cubic basis functions and a grid size large enough to ensure that the eigenvalues are accurate to 3 significant digits.\\
...
Before the eigenvalues of the beams can be compared, a method is required to be able to match the eigenvalues. In \cite{LVV09} the authors compare the modal shapes of the Timoshenko beam model to the modal shapes of the two-dimensional model.\\

Similar modal shapes implies that the eigenvalues can be matched up and compared. Eigenvalues relating to the shapes of the Timoshenko beams modes are called beam-type eigenvalues by \cite{LLV09}.

\subsubsection{Shapes relating to beam-type eigenvalues}
The following are examples of beam-type mode shapes for the displacement $w$ for the cantilever two-dimensional beam and cantilever Timoshenko beam. The figures are presented side-by-side.
...
The following are examples of non beam-type mode shapes for the displacement $w$ of the cantilever two-dimensional beam.
\begin{figure}[h!]
	\scalebox{.8}{
		\makebox[\textwidth][c]{
			\centering
...
Using this method all the relevant eigenvalues in this section were matched. The non-beam type eigenvalues are highlighted in gray in the tables.

\subsubsection{Shapes of the cross-section}
The Timoshenko beam theory differs from other one-dimensional beam theories by also including shear. It is assumed that the cross-sections need not remain perpendicular to the neutral axis of the beam. The cross-sections remain straight.\\

For the two-dimensional body, the shape of the cross-section of beam type eigenfunctions can deform into an a s-shape. Figure \ref{fig:s} shows an exaggerated representation of the s-shaped deformation. This is also given and explained in \cite{LVV09}.
\FloatBarrier
\begin{figure}[h!]
	\centering
...
\end{tikzpicture}
	\caption{S-shape deformation and average rotation.}\label{fig:s}
\end{figure}
The red line in figure \ref{fig:s} show how the average rotation of the cross-section is calculated in \cite{LVV09}.
\FloatBarrier

\subsubsection{Direct comparison of mode shapes}
Figure \ref{fig:w} shows the displacement $w$ of the cantilever Timoshenko beam and the cantilever two-dimensional beam directly compared.
...
Similarly Figure \ref{fig:phi} shows the direct comparison of the angle of the cross-section of the cantilever Timoshenko beam and the two-dimensional beam. The average rotation of the cross-section of the two-dimensional beam's mode shape is calculated as shown in figure \ref{fig:s}.
...
Figures \ref{fig:w} and \ref{fig:phi} show that for $\alpha = 4800$, the mode shapes are very close, even for the eigenvalues 8 (for Timoshenko beam) and 10 (for the two-dimensional beam). The authors of \cite{LVV09} obtained similar results for $\alpha = 300$ for the eigenvalues 6 (for Timoshenko beam) and 9 (for the two-dimensional beam). Therefore a direct comparison of the mode shapes are feasible for short and long beams. 

\subsubsection{Remark}
Due to the fact that any multiple of an eigenfunction is still an eigenfunction, the figures can be scaled. This is why no scale is indicated on the y-axis in \cite{LVV09}. Figures \ref{fig:w} and \ref{fig:phi} were scaled by eye.
...
This section continues the work of the article \cite{LVV09}. As mentioned by the authors of \cite{LVV09}, a comparison using a three-dimensional beam is desired. The two-dimensional beam model is an intermediate step to be able to investigate the validity of the Timoshenko beam model using a three-dimensional beam model as a reference. As mentioned by the authors, this will avoid some complexities.\\

The approach for this section remain similar to the previous section and the work of \cite{LVV09}. The eigenvalues for both models are calculated using FEM. The eigenvalues are then matched by sketching and comparing the mode shapes. Similar shapes indicate the the eigenvalues are comparable. The eigenvalues are then compared to determine how close the solutions of the models are.\\

As seen in the previous section, the two-dimensional model has eigenvalues not relating to beam type eigenvalues. There will also be eigenvalues not relating to beam type eigenvalues in this section. The results will include some of these eigenvalues, but the focus of our investigation remain with beam problems.

\subsection{The models}
Consider a cantilever two-dimensional beam, referred to as Problem 2D-1 in section \ref{ssec:2D_Model:ModelProblem} and a cantilever three-dimensional beam, referred to as Problem 3D-1 in section \ref{ssec:3D_Model:ModelProblems}.\\
...
As in \cite{LVV09}, the shape of the modes are compared. Similar mode shapes implies that the eigenvalues are comparable. Results for both beam and non-beam type eigenvalues are included. The focus still remains on beam type eigenvalues.

\subsubsection{Mode shapes relating to beam type eigenvalues.}
Figure \ref{fig:beam-2dv3d} show some examples of beam type mode shapes for the displacement $u$. These are shapes that can be obtained from the Timoshenko beam model.
...
Figure \ref{fig:nonbeam-2dv3d} show examples of mode shapes relating to non-beam type eigenvalues for the displacement $u$. These mode shapes are the non-beam type eigenvalues found in both the two-dimensional beam and the three-dimensional beam model.
