...
\begin{frame}{Literature Study}
    Article: 
    \textit{Comparison of linear beam theories} by Labuschagne, A., van Rensburg, N. F. J., and van der Merwe, A. J., published in \textit{Math. Comput. Model.}, volume 49, pages 20-30, in the year 2009.

    \only<2->{
        \begin{itemize}
            \item[-] Improves on other research by including the boundary conditions.
            \item[-] Compares a cantilever Timoshenko beam model to a cantilever two-dimensional model.
            \item[-] Comparison compares the eigenvalues and modal shapes of the two models (modal analysis).
        \end{itemize}
    }
    
\end{frame}

\begin{frame}{Timoshenko beam model}
...
\begin{frame}{Eigenvalue problem - FEM}
    \centering
    \includegraphics[scale=0.45]{Convergence.png} \\ % Adjust scale as needed
