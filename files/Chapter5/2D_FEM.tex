...
In this chapter, the Finite Element Method (FEM) is applied to the models of this dissertation, presented in chapter 1. The goal of this chapter is to obtain numerical formulas that can be used to calculate the eigenvalues and eigenvectors for the models. This is in preperation of chapter 6, where the eigenvalues and eigenvectors of the models are compared.

In chapter 4, a method was discussed to obtain the eigenvalues and eigenvectors for the Timoshenko beam model. Therefore it is not neccesary to apply the Finite Element Method on the Timoshenko model to obtain the eigenvalues and eigenvectors. The Finite Element Mehtod is applied to the two-dimensional and three-dimensional models.
...
Consider a two-dimensional elastic body clamped rigidly to a surface at $x_1 = 0$. The body is free-hanging on the other boundaries. Recall this is Problem 2D-1 in section \ref{ssec:2D_Model:ModelProblem}.
\FloatBarrier
\begin{figure}[h!]
	\centering
	\begin{tikzpicture}
