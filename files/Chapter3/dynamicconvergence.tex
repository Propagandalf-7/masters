...
In \cite{BV13}, a projection $P$ is defined so that $Pu \in V$. The error is
then split using this projection. Let $e(t) = Pu(t)- u_h(t)$ and $e_p(t) = u(t)
	- Pu(t)$ so that \label{sym:Pu}
\begin{eqnarray}
	e^h(t) = e_p(t) +e(t). \label{Error}
\end{eqnarray}

According to the authors of~\cite{BV13}, interpolation theory can be used to determine the error $e_p$. However to obtain an error estimate for $e$ is not as trivial. This is where the use of the Finite Element Analysis comes in.

To simplify the proof, the problem is divided into a semi-discrete problem and a fully discrete problem.
...
In \eqref{error_e}, the authors divide the error $e$ into a semi-discrete term
and a fully discrete term. The two terms are investigated as seperate problems
in \cite{BV13}.\\
...
In this section we will show how the error estimate in Theorem 2 is obtained by
the authors of \cite{BV13}.\\
...
From assumption \textbf{B1}, $Pu \in C^2(J)$ and by the definition $P$, $P$ is
a projection with respect to the inner product $b$.\\
...
Using the definition of the error $e^h$ while also adding and subtracting
$Pu(t)$ in the bilinear form $b$, we can write
\begin{eqnarray*}
	c(e_h''(t),v) + a(e_h'(t),v) + b(u(t) - Pu(t),v) + b(Pu(t) +u_h(t),v)- = 0.
\end{eqnarray*}

It then follows from the definitions of the error $e$ and the projection $P$
that
\begin{eqnarray*}
	c(e''(t),v) + a(e'(t),v) + b(e(t),v) = 0
\end{eqnarray*}

This equation \eqref{DC_E3} is presented in \cite{BV13} as a proposition.
\newtheorem{DC_Prop1}{Proposition}
\begin{DC_Prop1}
	If $u$ is the solution of Problem GVar and $u$ satisfies assumption \textbf{B1}, then
	\begin{eqnarray}
		c(e_h''(t),v) + a(e_h'(t),v) + b(e(t),v) = 0 \ \ \ \textrm{ for all } v \in V.
	\end{eqnarray}
\end{DC_Prop1}

Using this proposition, the authors of \cite{BV13} derive a generalized error
estimate for the error $e = Pu - u_h$.
...
In chapter 2, it was explained that the models of this dissertation have weak
damping, i.e. $a = 0$. Therefore, the error estimate in Lemma 1 can be
simplified. This is given in \cite{BV13} as a corollary.

\newtheorem{col_1}{Corollary}
\begin{col_1}
	If $u$ is the solution of Problem GVar with $a = 0$ and $u$ satisfies assumption \textbf{B1}, then
	\begin{eqnarray}
		\Vert e_(t)\Vert_{W} \leq \sqrt{2} \Vert e(0) \Vert_W + 2T||e'_h(0)||_W + 4\sqrt{T}\max_{t \in [0,T]} \Vert e'_p(t) \Vert_W.
	\end{eqnarray}
\end{col_1}

\begin{proof}
	Assume that there is no damping other than the weak damping, i.e. $a = 0$. The the equation in proposition 1 becomes
	\begin{eqnarray*}
		c(e_h''(t),v) + b(e(t),v) = 0 \ \ \ \textrm{ for all } v \in V.
	\end{eqnarray*}
	
	If $v \in S^h[0,T]$, then 
	\begin{eqnarray*}
		c(e_h''(t),v(t)) + b(e(t),v(t)) = 0.
	\end{eqnarray*}

	Let $v'(t) = c(t)$, then 
	\begin{flalign}
		\frac{d}{dt}\left[\frac{1}{2}c(e,e)-\frac{1}{2}b(v,v)-c(e_h,v) \right] &= c(e',e) - b(e,v)- c(e''_h, v)-c(e_h',v')\nonumber\\
		&= c(e'_p,e). \label{DC_E4}
	\end{flalign}

	Choose $v(\tau) = 0$ for some $\tau \in [0,T]$, and integrate \eqref{DC_E4} between $0$ and $\tau$ to obtain
	\begin{eqnarray*}
		a
	\end{eqnarray*}
...
From Theorem 1, it follows that the solution $u$ of Problem GVar satisfies $u
	\in C^1([0,T]; V) \cap C^2((0,T);V)$. \textcolor{red}{This is a weak assumption
	made in \cite{BV13}.}\\
...
Let $\Pi$ denote the interpolation operator such that $u_0^h = \Pi u_0$ and
$u_1^h = \Pi u_1$.
...
\subsubsection*{Problem GVar$^\mathbf{{h}}$-D} Find a sequence
$\{u_{k}^{h}\}\subset S^{h}$ such that for $k=0,1,2$, . . . , $N-1,$
\begin{eqnarray}
...
In the proof of Proposition 3, the authors of \cite{BV13} show that \eqref{PP}
in Problem GVar$^h$-D can be rewritten as
\begin{eqnarray}
...
Using \eqref{PPP} and the definition of the standard finite element matrices,
Problem GVar$^h$-D can be reformulated into a algorithm for the Finite Element
Method. This is the fully discrete form of Problem GVar and is represented in
Problem FD.\\
\textcolor{red}{******************************************************}\\

Further results and the standard Finite Element Matrices, let the authors of
\cite{BV13} rewrite Problem GVar$^\mathbf{{h}}$-D into an algorithm for the
Finite Element Method. This is represented as Problem FD
...
In this form, the problem is fully discrete. This form also allow the authors
of \cite{BV13} to obtain an error estimate for the fully-discrete problem.
