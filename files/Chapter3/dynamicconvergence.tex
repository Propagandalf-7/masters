...
\insubfile{\AtEndDocument{\bibliography{../../references.bib}}}
...
\subsection{Galerkin approximation of Problem GVar}\label{subsec:galerkin-approximation-of-problem-gvar}
Recall from Chapter 2 that $V$ is the closure of the test function space $T$ in
the Hilbert space $H$.

Suppose $S^h$ is a finite n-dimensional subspace of V. Let $\{\phi_1, \
	\phi_2,...,\phi_n\}$ be a set of linear independent basis functions such that
$S^h = \textrm{span}(\{\phi_1, \phi_2, ..., \phi_n \})$ and $\phi_i \in
	C^{(1)}$ for all $i$.
...
The authors of \cite{BV13} mention that interpolation theory can be used to determine the error $e_p$. However to obtain an error estimate for $e$ is not as trivial. This is where the authors use the Finite Element Analysis, which is used to obtain an error estimate for $e$.
