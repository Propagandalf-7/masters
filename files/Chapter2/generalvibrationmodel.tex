...
To apply the theory from \cite{VV02}, complete function spaces are required. It is known that $L^2(0,1)$ is a complete function space and hence $X$ is complete.\\
 ...
 The Sobolev space $H^1(0,1)$ is all the functions in $L^2(0,1)$ with at least a first-order weak derivative. $H^1(0,1)$ is complete (see Appendix A).\\
  ...
  The test functions $T[0,1]$ do not form a complete space. Let $V(0,1)$ be the closure of {$F_1(0,1)$} in $H^1(0,1)$. The product space {$V(0,1)\times V(0,1)$} is denoted by $V$ and is complete. $V$ is called the energy space.\\
...
As mentioned before, if $a = 0$ then the inequalities in \eqref{weak_damping} hold trivially. However, the operator A is defined by the operator $\Lambda$ and in the definition of $\Lambda$, the form $a$ is used (see \eqref{author_mapping}).\\

\vspace{10mm}
 derivation of this linear operator remains unchanged as presented in \cite{VV02} for $a$ identically zero.\\
 \vspace{10mm}
 
  To apply Theorem 2, the admissible initial conditions are $u_0 \in E_b$ and $u_1 \in V$. As mentioned $\langle u_0, u_1 \rangle \in D(A)$. From semigroup theory, it follows that $u(t) \in D(A)$ for each $t$. Therefore $u(t) \in E_b$ and $u'(t) \in V$ for each $t$.\\


\textcolor{red}{Volgende Rondte}\\
 Using the assumptions \textbf{A1}-\textbf{A4}, they prove that this linear operator $A$ is an infinitesimal generator of a $C_0$-semigroup of contractions and the domain of $A$ is dense in $H$.
...
\textcolor{red}{ALLES NOG NIE BEWYS NIE - moet nog doen}

 To apply the theory, Problem T-2W must satisfy assumptions \textbf{A1}-\textbf{A4} and the initial values must be admissible. 

\subsubsection{Assumption A1}\label{sssec:existence:AssumptionA1}
It is clear that $W$ is dense in $X$. (The set $W = X$).\\

Let $u \in C_0^\infty(0,1)$. Then $u(0) = u(1) = 0$ and therefore $u \in T(0,1)$ and $C_0^\infty(0,1) \subset T(0,1)$. Recall that $V(0,1)$ is the closure of $T(0,1)$ in $H^1(0,1)$.\\

So it follows that $C_0^\infty(0,1) \subset V(0,1) \subset H^1(0,1)$. And since $C_0^\infty(0,1)$ is dense in $L_2(0,1)$, both $V(0,1)$ and $H^1(0,1)$ are dense in $L_2(0,1)$. Therefore $V(0,1)\times V(0,1)$ is dense in $X = L_2(0,1) \times L_2(0,1)$.

\subsubsection{Assumption A2}\label{sssec:existence:AssumptionA2}
	From the definition of the bilinear form $c$, 
\begin{eqnarray*}
...
To show that assumption \textbf{A3} holds, it is required to show that there exists a positive constant $C_{V}$ such that $\Vert v\Vert_{W} \leq C_{V}\Vert v\Vert_{V}$ for each $v \in V$.\\

Let $f \in V$, then $f_1(0) = f_2(0) = 0$.
\begin{eqnarray}
...
\end{eqnarray}
 ...
 Using the Corollary, and from the definition of the norm $||\cdot||_X$, the following Poincar\'e-Type inequality for the norm $||\cdot||_X$ can be obtained.
\begin{eqnarray*}
	||f||_X^2 & \leq & ||f_1'||^2 + ||f_2'||^2 \\
	& = & ||f_1'||^2 -||f_2||^2 + ||f_2'||^2 +||f_2||^2\\
	& \leq & 2||f_1'-f_2||^2 + 2||f_2'||^2.
\end{eqnarray*} \sout{by Proposition 2 and the reverse triangle inequality.}\\

Using \eqref{b} and the inequality above, it follows that
\begin{eqnarray}		
	b(f,f)	& \geq & \frac{1}{2}\min\left \{1, \frac{1}{\beta}\right \} ||f||_{X}^2. \label{b2}
\end{eqnarray} for all $f \in V$.\\

Since $b$ is the bilinear form for $V$ and $W = X$, \eqref{b2} is equivalent to \[||v||^2_V \geq C^{\frac{1}{2}}_V ||v||^2_W\] for all $v \in V$ with $\displaystyle C^{\frac{1}{2}}_V = \frac{1}{2}\min\left \{1, \frac{1}{\beta}\right \} > 0$. It follows that $\displaystyle||v||_W \leq C_V||v||_V$ for all $v \in V$.\\ 

\sout{Since $b$ is a bilinear form and from the inequality above, $b$ is a inner-product for $V$. Using the inequality \eqref{C} results in $||v||^2_V \geq  C_3 ||w||_{W}^2$ for all $v \in V$, $w \in W$. With $C_3 =\frac{1}{2}\min\left \{1, \frac{1}{\beta}\right \} > 0$. The result follows by taking the square on both sides of the inequality.}\\

Consider \textbf{Assumption A4}. In Problem T-2W, it is clear that the bilinear form $a$ is identically zero. As mentioned before... \textcolor{red}{As mentioned before, if $a = 0$, the weak damping as well as assumption \textbf{A4} is satisfied.}\\


\section{Modal analysis}\label{sec:existence:ModalAnalysis}
\subsection{Timoshenko beam}
In this dissertation, the eigenvalues and eigenvectors of the models presented are required. To understand what is meant by modal analysis, it is in the first place necessary to consider modes of vibration. And to understand how modal analysis is applied, a Timoshenko beam is used as an example. Specifically, Problem T-1.\\
...
\end{eqnarray*}
Consider a trial solution $w(x,t) = T(t)\tilde{w}(x)$ with $\phi(x,t) = T(t)\tilde{\phi}(x)$. Simple substitution confirms that this trial solution satisfies the partial differential equations and the boundary conditions for Problem T-1.\\
...
The solution to Problem T-1E is a pair of functions $\langle \tilde{w} \ \tilde{\phi} \rangle$ called the eigenfunction with corresponding eigenvalue $\lambda$.\\
...
\textcolor{red}{Res moet dalk out of skuif.*******************************}\\
It is now possible to test the trial solutions $w(x,t) = T(t)\tilde{w}(x)$ and $\phi(x,t) = T(t)\tilde{\phi}(x)$. Consider the right hand side of the variational form of Problem T-1V with $Q=0$. Then substituting the pair of trial solutions:
\begin{eqnarray*}
	-\int_{0}^{1}\partial_x w v'  + \int_{0}^{1} \phi v' &=& -T\int_{0}^{1} \tilde{w}' v'  + T\int_{0}^{1} \tilde{\phi} v',\\
	- \int_{0}^{1} \alpha \partial_x w \psi + \int_{0}^{1} \alpha \phi \psi - \frac{1}{\gamma} \int_{0}^{1}\partial_x \phi \psi' &=& - T\int_{0}^{1} \alpha \tilde{w}' \psi + T\int_{0}^{1} \alpha \tilde{\phi} \psi - \frac{1}{\gamma} T\int_{0}^{1} \tilde{\phi}' \psi'.
\end{eqnarray*}
Similarly for the left hand side and using \eqref{eq:1D_Model:ModalAnalysisT}:
\begin{eqnarray*}
	\int_{0}^{1} \partial_{t}^{2} w v &= \int_0^1 \ddot{T} \tilde{w} v &= T\int_0^1 \lambda \tilde{w} v,\\
	\int_{0}^{1} \partial_{t}^{2} \phi \psi &= \int_0^1 \ddot{T} \tilde{\phi} \psi &= T \int_0^1 \lambda \tilde{\phi} \psi.
\end{eqnarray*}

This shows that the pair of trial solutions $\langle {w} \ {\phi} \rangle$ are solutions of the variational problem Problem T-1V with $\langle \tilde{w} \ \tilde{\phi} \rangle$ the eigenvalues with corresponding eigenvalue $\lambda$ from the variational eigenvalue problem Problem T-1EV. This was accomplished without using the method of separation of variables and thus avoiding any contention on division by zero.\\
...
\subsection{General vibration problem}
	...
	\textbf{********* MOET [CVV18] in bronnelys sit}\\

Consider Problem XX in Section 2.2 \textbf{*** Sy reference is \eqref{GV_PB1}}\\

Given $g \in C((a,b);W)$, find $u$ such that for each $t > 0$, $u(t) \in V$, $u \in C^2((a,b);W)$ and
\begin{eqnarray*}
	c(u''(t),v) + b(u(t),v) = c(g(t),v).
\end{eqnarray*}

Now suppose there exists $\lambda$ and $x \in V$, $x \neq 0$ such that
\begin{eqnarray*}
