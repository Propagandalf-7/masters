...
Solutions to Problem T-1E is a pair of functions $\langle \tilde{w}, \tilde{\phi} \rangle$ called the eigenfunction with corresponding eigenvalue $\lambda$. Substitution confirms that $\tilde{w} = \sin(k\pi x)$ and $\tilde{\phi} = \cos(k \pi x)$ are possible solutions of the differential equations and satisfy the boundary conditions.

Due to this periodicity of these solutions for the eigenvalue problem Problem T-1E, it has infinitely many solutions $\langle \tilde{w}_n, \tilde{\phi}_n \rangle$ and corresponding sequence of eigenvalues $\lambda_n$. The vector $\langle 0, 1 \rangle$ also satisfies the eigenvalue problem with boundary conditions. This solution is called the constant solution.

A systematic approach to obtain the eigenvalues and eigenfunctions is given in \cite{VV06}. This is discussed in in Section 4.2. From \cite{VV06}, the following theorem can be derived and confirms the validity of our trial solutions for the eigenvalue problem.
...
This section is a discussion of a general vibration problem. The discussion will refer to the article \cite{CVV18}. Consider Problem GVar defined in Section \ref{ssec:existence:VariationlApproach}. The authors of \cite{CVV18} do take damping into consideration, which is not covered in this dissertation. Assume there is no damping and no forcing. 
...
Since this differential equation is a simple second order differential equation, T(t) has the following possible solutions as given in \cite{CVV18}:
\begin{flalign}
	T_n(t) &= e^{-r_nt}\left[ A_n \cos(\omega_n t) + B_n \sin(\omega_n t) \right] \quad \textrm{ if } \lambda > 0, \label{lambda_1}\\
	T_n(t) &= e^{-r_nt}\left[ A_n + B_n t \right] \quad \textrm{ if } \lambda = 0, \label{lambda_2}\\
	T_n(t) &= e^{-r_nt}\left[ A_n \cosh(\mu_n t) + B_n \sinh(\mu_n t) \right] \quad \textrm{ if } \lambda < 0. \label{lambda_3}
\end{flalign}

From assumptions \textbf{A2} and \textbf{A3}, the bilinear forms $b$ and $c$ are positive definite. And since we assumed that there is no damping ($a=0$), $\lambda$ cannot be negative. Therefore, the only possible solutions for $T_n(t)$ is \eqref{lambda_1} and \eqref{lambda_2}.
