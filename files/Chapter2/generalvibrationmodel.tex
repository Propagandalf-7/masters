...
Consider Problem GVar defined in Section \ref{ssec:existence:VariationlApproach}. In this section, assume there is no damping and no forcing.
...
Therefore $\frac{T''(t)}{T(t)}$ must be a constant. Suppose that $\frac{T''(t)}{T(t)} = -\lambda$. The existence of such a $\lambda$ is uncertain at this point. To investigate the existence of such a $\lambda$, consider the following eigenvalue problem.
...
A solution to the eigvalue problem consists of an eigenvalue $\lambda$ with corresponding eigenvector $x$. 
...
(Note that this assumption expands on the assumptions already made in Section 2.2.)

Buidling on assumption \textbf{A5}, the authors of \cite{CVV18} prove that there exists a eigenvalue $\lambda$ and a corresponding eigenvector $x$ such that the eigenvalue problem has a solution. The authors also prove that the eigenvalues are positive, and the corresponding eigenvectors are orthogonal in $W$ (the bilinear form $c$). In fact it is also orthogonal with respect to the bilinear form $b$, hence
\begin{eqnarray*}
	b(x_i, x_j) = \lambda_i c(x_i, x_j) = 0, \ \ \ \ \textrm{ for each } \ i \neq j.
\end{eqnarray*}

The sequence of eigenvalues ${\lambda_i}$ is an infinite sequence with $\lambda_n \rightarrow \infty$ as $n \rightarrow \infty$. The sequence of eigenvectors ${x_i}$ forms an orthogonal basis in $W$, which means that any element of $W$ can be represented as a linear combination of the eigenvectors.

\textcolor{red}{Onseker oor die volgede deel oor die Fourier Coefficients. Moet net nog daaroor dink.}

Following this result, the authors explain that any $u \in V$ can be represented as a linear combination of the eigenvectors $x_i$ with coefficients $a_i$. These coefficients are Fourier coefficients of $u$ with respect to the basis $x_i$. Therefore, for any $u \in V$,
\begin{eqnarray*}
...
\begin{flalign*}
	T_n(t) &= e^{-r_nt}\left[ A_n \cos(\omega_n t) + B_n \sin(\omega_n t) \right],\\
	T_n(t) &= e^{-r_nt}\left[ A_n + B_n t) \right],\\
	T_n(t) &= e^{-r_nt}\left[ A_n \cosh(\mu_n t) + B_n \sinh(\mu_n t) \right].
\end{flalign*}
...
Substitution shows that this trial solution is a solution to \eqref{eq:existence:ProblemGVarHom} if and only if you can find a real number $\lambda$ such that 
\begin{eqnarray}
	T''  + \lambda T = 0,
\end{eqnarray}
...
\textcolor{red}{*************** Need to check everything below this ******************}
