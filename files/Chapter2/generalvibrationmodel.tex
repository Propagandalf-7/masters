...
\textcolor{red}{Commonly at this point, the method of seperation of variables is applied. This splits the partial differential equation into an eigenvalue problem and a differential equation. Instead of using this method, a formulation of the eigenvalue problem and differential equation will be assumed and we'll show that they satisfy the trial solutions.}\sout{The method of separation of variables leads to an eigenvalue problem.}

For the pinned-pinned beam, the eigenvalue problem is referred to as Problem T-1E.
...
Solutions to Problem T-1E is a pair of functions $\langle \tilde{w}, \tilde{\phi} \rangle$ called the eigenfunction with corresponding eigenvalue $\lambda$. \textcolor{red}{Substitution confirms that $\tilde{w} = \sin(k\pi x)$ and $\tilde{\phi} = \cos(k \pi x)$ are solutions of the differential equations and satisfy the boundary conditions.}
...
\textcolor{red}{Die deel copy vanaf later in die section}

\textcolor{red}{********************************************************}

It is well known that the eigenvalue problem like Problem T-1E has infinitely many solutions $\langle \tilde{w}_n, \tilde{\phi}_n \rangle$ and corresponding sequence of eigenvalues $\lambda_n$. For the pinned-pinned beam (Problem T-1), $\lambda_n > 0$ for all $n > 0$ (this is also true for Problem T-2 and Problem T-4). A convenient reference is \cite{VV06}.
...
\textcolor{red}{Los theorem voorlopig hier. Alles onder die theorem is soortgelyk aan wat hier bo is. Was gedoen op die variational form van Problem T1.}\\
...
It is well known that the eigenvalue problem like Problem T-1EV has infinitely many solutions $\langle \tilde{w}_n \ \tilde{\phi}_n \rangle$ and corresponding sequence of eigenvalues $\lambda_n$. For Problem T-1 (Problem T-2 and Problem T-4 also), $\lambda_n > 0$ for all $n > 0$. A convenient reference is \cite{VV06}.\\
...
The article \cite{VV06} provides a rigorous method to solving the eigenvalue problem for Timoshenko beam models of different boundary conditions. This method and examples form \cite{VV06} is discussed in Chapter 4.

\subsection{General vibration problem new}
Using the pinned-pinned beam as an example, in the previous section it was shown that a formal series solution can be derived that satisfies the boundary conditions. In this section, a more general vibration problem is considered that also includes initial conditions.
