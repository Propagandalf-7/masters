...
The fifth chapter is preparation for the comparisons of the models in this dissertation. In chapter 4, we discussed a method to obtain to obtain the exact solutions for the eigenvalue problem for the Timoshenko beam model. For the other models a different approach is required. For each of the models, the Galerkin Approximation is derived. The reference configuration is the divided into discrete elements. We then apply piece-wise Hermite cubic functions and use the Finite Element Method to obtain a initial value problem for each of the models consisting of a system of ordinary differential equations. An eigenvalue problem is then derived from this IVP. Using a computer program, this eigenvalue problem can be solved and the eigenvalues and eigenvectors can be calculated. An investigation into the rate of convergence was conducted to make sure that the results are accurate enough.

In the final chapter, the work of section 4.5 is extended. The validity of the two-dimensional beam model and the Reissner-Mindlin plate model is investigated. Using the work done in chapter 5, the eigenvalue problems for a cantilever two-dimensional beam model, cantilever three-dimensional beam model and a cantilever Reissner-Mindlin plate model is solved using the Finite Element Method and a computer program. The same approach as section 4.5 is followed. The mode shapes are plotted and the eigenvalues are matched up. Careful consideration to the eigenvalues and mode shapes was taken to ensure that the eigenvalues are relevant to beam-type (or plate-type) eigenvalues and mode shapes. To investigate the two-dimensional cantilever beam, it is compared to a three-dimensional beam model. 

The three-dimensional more complex and contains non-beam type eigenvalues that are shared with the two-dimensional model as well as non-beam type eigenvalues that are not present in the two-dimensional model. The eigenvalues not present in the two-dimensional model are omitted from the results and all the eigenvalues shared between the models are included with the focus on the beam-type eigenvalues. The results shows that the depth of the three-dimensional beam is very important the results. The cases considered covers a range of possibilities for realistic beam models for use in practical applications.

Similarly the Reissner-Mindlin plate model is compared to the three-dimensional plat model. The results are restricted to cases where the width of the models 
